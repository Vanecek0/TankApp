    ...
    return (
        <View style={{ backgroundColor: isDark ? Colors.dark.background : Colors.light.background }} className={`${className} flex outline-none border-none py-3 px-5 gap-2 flex-row items-center justify-between`}>
            <View className="flex-row items-center gap-3">
                <ScaledText className='rounded-full w-10 h-10 text-center flex justify-center items-center align-middle aspect-square' style={{ backgroundColor: "lightgray", fontWeight: "bold" }} size='sm'>A</ScaledText>
                <ScaledText size="base" className="font-bold" isThemed={true}>Audi TT</ScaledText>
            </View>
            <View className="flex-row gap-2">
                <Link href={"/tank"} className="flex items-center"><Icon name="bell" color={Colors.inactive_icon} style={{ width: 30, height: 30 }} /></Link>
                <Link href={"/tank"} className="flex items-center"><Icon name="settings" color={Colors.inactive_icon} style={{ width: 30, height: 30 }} /></Link>
                <TouchableOpacity
