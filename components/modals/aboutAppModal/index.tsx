        ...
        <View className='max-h-full' style={{ ...spacing.borderRadius(12) }}>
            ...
            <ScrollView style={{ ...spacing.px(24), ...spacing.mb(24), ...spacing.gap(12) }} className='flex'>
                ...
                <View style={{ ...spacing.gap(12), ...spacing.py(24) }}>
                    ...
                    <View className='flex-row items-start' style={{ ...spacing.borderRadius(6), ...spacing.p(12), backgroundColor: isDark ? Colors.background.dark : Colors.background.light }}>
                        <View className="justify-center items-center" style={{ width: getScaleFactor() * 35, height: getScaleFactor() * 35 }}>
                            <Icon name="tank" color={Colors.icon.disabled} size={getScaleFactor() * 25} />
                        </View>
                        <View style={{ ...spacing.px(5) }} className="justify-center">
                            ...
                            <ScaledText size='sm' isThemed={true} style={{ color: Colors.text.muted }}>Veškeré záznamy z takování</ScaledText>
                        </View>
                    </View>
                    <View className='flex-row items-start' style={{ ...spacing.borderRadius(6), ...spacing.p(12), backgroundColor: isDark ? Colors.background.dark : Colors.background.light }}>
                        <View className="justify-center items-center" style={{ width: getScaleFactor() * 35, height: getScaleFactor() * 35 }}>
                            <Icon name="average" color={Colors.icon.disabled} size={getScaleFactor() * 25} />
                        </View>
                        <View style={{ ...spacing.px(5) }} className="justify-center">
                            <ScaledText size='base' className='font-bold' isThemed={true}>Detailní statistiky</ScaledText>
                            <ScaledText size='sm' isThemed={true} style={{ color: Colors.text.muted }}>Analýza spotřeby, cen a další...</ScaledText>
                        </View>
                    </View>
                    <View className='flex-row items-start' style={{ ...spacing.borderRadius(6), ...spacing.p(12), backgroundColor: isDark ? Colors.background.dark : Colors.background.light }}>
                        <View className="justify-center items-center" style={{ width: getScaleFactor() * 35, height: getScaleFactor() * 35 }}>
                            <Icon name="car_repair" color={Colors.icon.disabled} size={getScaleFactor() * 25} />
                        </View>
                        <View style={{ ...spacing.px(5) }} className="justify-center">
                            <ScaledText size='base' className='font-bold' isThemed={true}>Servis vozidla</ScaledText>
                            <ScaledText size='sm' isThemed={true} style={{ color: Colors.text.muted }}>Servisní záznamy, kontroly ...</ScaledText>
                        </View>
                    </View>
                </View>
                ...
                <View style={{ ...spacing.gap(12), ...spacing.py(24) }}>
                    ...
                    <View className="flex-row items-start" style={{ ...spacing.gap(8) }}>
                        ...
                        <View style={{...spacing.gap(8)}}>
                            ...
                            <ScaledText size='sm' isThemed={true} style={{ color: Colors.text.muted }} >Volnočasový vývojář webových a mobilních aplikací, který se nebojí experimentovat a zkoušet nové technologie. ☺️</ScaledText>
                            <View className='flex-row items-center' style={{ ...spacing.gap(8) }}>
                                <View className="flex items-center justify-center">
                                    <Icon name='github' color={Colors.icon.disabled} size={getScaleFactor() * 20} />
                                </View>
                                <View>
                                    <ScaledText size='base' isThemed={true} style={{ color: Colors.text.muted }}>GitHub</ScaledText>
                                </View>
                            </View>
                        </View>
                    </View>
                ...
                <View style={{ ...spacing.gap(12), ...spacing.py(24) }}>
                    <View className='flex-row items-center' style={{ ...spacing.gap(8) }}>
                        <View className="flex items-center justify-center">
                            <Icon name='external_link' color={Colors.icon.disabled} size={getScaleFactor() * 20} />
                        </View>
                        <View>
                            <ScaledText size='base' isThemed={true} style={{ color: Colors.text.muted }}>Podmínky použití</ScaledText>
                        </View>
                    </View>
                </View>

                <View className='border-t-[1px]' style={{ borderColor: isDark ? Colors.background.dark : Colors.background.light, ...spacing.pt(24), ...spacing.gap(8) }}>
                    <ScaledText size='sm' isThemed={true} className='text-center'>
                        Vytvořeno s ♥ v České republice
                    </ScaledText>
                    <ScaledText size='xs' isThemed={true} style={{ color: Colors.text.muted }} className='text-center'>© 2025 Pavel Vaněček. Všechna práva vyhrazena.</ScaledText>
                </View>
